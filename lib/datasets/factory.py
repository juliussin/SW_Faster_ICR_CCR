...
from datasets.itri import itri
...
__sets["itri_train"] = lambda split="train": itri(split)
__sets["itri_test"] = lambda split="test": itri(split)

for city in ["Tokyo", "Taipei", "Rio", "Rome"]:
    for split in ["train", "test", "fine_tuning"]:
        name = "nthu_{}_{}".format(city, split)
        __sets[name] = lambda city=city, split=split: nthu(split, city)
